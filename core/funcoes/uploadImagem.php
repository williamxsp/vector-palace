...
define('NICUPLOAD_PATH', '../../img/upload'); // Set the path (relative or absolute) to
                                      // the directory to save image files
                                      
define('NICUPLOAD_URI', 'img/upload');   // Set the URL (relative or absolute) to
...
if(!function_exists('json_encode')) {
    die('{"error" : "Seu navegador não suporta essa função."}');
}
...
if($_SERVER['REQUEST_METHOD']=='POST') { // Upload is complete
    ...
    if(!is_dir(NICUPLOAD_PATH) || !is_writable(NICUPLOAD_PATH)) {
        nicupload_error('O diretório '.NICUPLOAD_PATH.' precisa ser criado e ter permissões de escrita para continuar');
    }
    ...
    if(!$file) {
        nicupload_error('Arquivo precisa ser menor do que '.bytes_to_readable($max_upload_size));
    }
    ...
    if(!move_uploaded_file($image, $path)) {
        nicupload_error('Não foi possível salvar o arquivo');
    }
    ...
    $check = $_GET['check'];
    if(!is_numeric($check)) {
        nicupload_error('Upload inválido');
    }
